...
public class FightingGameApp extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        // Step 1: Display Logo with Color Changes
        displayLogoWithColorChange(() -> {
            // Step 2: Prompt user to continue or exit after logo display
            promptUserChoice();
        });
    }
    ...
    /**
     * Displays the ASCII logo with color transitions.
     */
    private void displayLogoWithColorChange(Runnable onLogoComplete) {
        ...
        for (String color : colors) {
            // Add extra lines to simulate clearing the console
            for (int i = 0; i < 25; i++) {
                System.out.println();
            }
            
            // Print the logo in the current color
        ...
        try {
            Thread.sleep(900); // 1 second pause for each color change
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }

    // Logo display is done, proceed to next screen
    onLogoComplete.run();
}
    ...
    private void showCharacterSelectionScreen() {
        ...
        // Set up onReady callback to start the game with selected characters
        characterSelectScreen.setOnReady((player1, player2) -> {
            startGameWithCharacters(player1, player2);
        });
    ...
    /**
     * Starts the actual game/fighting scene.
     */
